...
class _MemberDialogContentState extends State<_MemberDialogContent> {
  ...
  Widget _buildRegimeSelector(BuildContext context, String current, Function(String?) onChanged) {
    final List<Map<String, dynamic>> options = [
      {'val': 'Onnivoro', 'icon': Icons.restaurant_rounded, 'desc': 'Alimentazione completa'},
      {'val': 'Vegetariano', 'icon': Icons.egg_alt_rounded, 'desc': 'No carne e pesce'},
      {'val': 'Vegano', 'icon': Icons.eco_rounded, 'desc': 'Esclude derivati animali'},
      {'val': 'Chetogenico', 'icon': Icons.bolt_rounded, 'desc': 'Bassi carboidrati'},
      {'val': 'Paleo', 'icon': Icons.hiking_rounded, 'desc': 'Alimentazione ancestrale'},
    ];
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          selectedItemBuilder: (ctx) => options.map((o) => Row(
            children: [
              Icon(o['icon'] as IconData, size: 18, color: BC.getPrimary(context)),
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: Icon(o['icon'] as IconData, size: 18, color: BC.getPrimary(context)),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(o['val'] as String, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                        Text(o['desc'] as String, style: TextStyle(fontSize: 9, color: BC.getTextSub(context))),
                      ],
                    ),
                  ),
                ],
